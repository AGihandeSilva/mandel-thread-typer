...
ToolsOptionsWidget::ToolsOptionsWidget(RenderThread *masterThread, MandelbrotWidget* mainWidget, SettingsHandler& settingsHandler)
    : sliderTitle(nullptr), threadCountSlider(nullptr), numPassesTitle(nullptr), threadAlgorithmTitle(nullptr),
      ...
      numPassValue(masterThread != nullptr ? masterThread->getRunningNumPasses() : defaultNumPassesValue),
...
{
    const MQintVector& passesOptions = masterThread->getPossiblePassValues();
...
void ToolsOptionsWidget::setAlgorithmInGUI()
{
    ...
    threadAlgorithmButtons->buttons()[toUnderlyingType(currentAlgorithm)]->setChecked(true);
}
...
void ToolsOptionsWidget::addHorizontalLine()
{
    const int spacingAroundHLines = 20;

    toolsOptionsLayout->addSpacing(spacingAroundHLines);

    QFrame* frame = new QFrame(this);
    frame->setFrameShape(QFrame::HLine);
    frame->setFrameShadow(QFrame::Sunken);
    toolsOptionsLayout->addWidget(frame);
...
void ToolsOptionsWidget::createThreadSlider()
{
    ...
    threadCountSlider->setRange(MIN_NUM_WORKER_THREADS, maxNumThreads);
