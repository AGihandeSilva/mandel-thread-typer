...
public slots:
    void configureThreadInfo(int numberOfThreads, threadState currentState = threadState::idle);
    void setThreadState(uint threadIndex, int state);

private:
    void initialiseIndicators();
    void setThreadState(uint threadIndex, threadState state);
    ...
    std::array<QLabel*, MAX_NUM_WORKER_THREADS> threadStateIndicators {};
};
